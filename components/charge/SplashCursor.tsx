'use client';
/* eslint-disable @typescript-eslint/no-explicit-any, prefer-const, react-hooks/exhaustive-deps, @typescript-eslint/no-unused-vars */
/**
 * SplashCursor — WebGL fluid-dynamics trace for the charge ritual.
 * STATUS: implemented — ported from v1 (git show main:.../SplashCursor.tsx),
 * adapted: emotionColor tint, onActivity callback for the completion meter,
 * reduced-motion guard. Cleanup (cancelAnimationFrame + listener removal)
 * was already present in the source.
 * Third-party-derived shader code — exempt from our lint rules (see CONVENTIONS).
...
interface SplashCursorProps {
  ...
  BACK_COLOR?: ColorRGB;
  TRANSPARENT?: boolean;
  /** Hex color the fluid is tinted toward (the chosen emotion). */
  emotionColor?: string;
  /** Fired on pointer movement so the ritual can measure engagement. */
  onActivity?: () => void;
}
...
export function SplashCursor({
  ...
  useEffect(() => {
    ...
    // Respect reduced-motion: skip the simulation entirely; ChargeRitual
    // still drives completion via its own timer/affordance.
    if (
    ...
    function generateColor(): ColorRGB {
      // Tint every splat toward the chosen emotion color, with a little
      // brightness shimmer so the fluid still feels alive.
      ...
      const shimmer = 0.12 + Math.random() * 0.06;
      return { r: br * shimmer, g: bg * shimmer, b: bb * shimmer };
    }
    ...
    // Named event handlers so they can be properly removed on cleanup
    ...
    // Cleanup: cancel the loop and remove all listeners on unmount
    return () => {
