...
| M1 | Auth + profile | Auth.js Google login, create-profile, settings, session gate, theming live — swaps the dev-identity shim in lib/auth | New user can sign in, onboard, pick theme, sign out, return | **profile + theming landed 2026-06-15 against the dev shim** (create-profile + ProfileForm, PATCH /api/users/[id], live theme/colorTheme preview via UserProvider, getCurrentUser in lib/auth). **Google OAuth + real sessions + sign-out still pending** — the LAST milestone, ADR-009. |
...
| M3 | Grimoire + library | Book shell (ribbon tabs), library grid + Closed cases view, sigil page, rename, destroy (status flip) — local-first over lib/sigil/localStore | Full read/manage lifecycle of own sigils | **core done 2026-06-12**; **social/profile API + UI landed 2026-06-15** (DB-backed): users search/profile/PATCH/DELETE, follows GET/POST/DELETE (incl. mutual = SigiFriends), sigil share; UserSearch/FollowButton/FriendsList wired on /grimoire/friends (now linked in Menu); grimoire LeftPage/RightPage implemented but the default-spread mount is still deferred. |
